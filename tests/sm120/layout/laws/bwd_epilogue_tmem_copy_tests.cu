...
__global__ void bwd_epilogue_tmem_copy_compile() {
  using LoadOp = cute::SM100_TMEM_LOAD_16dp32b32x;
  using X = cute::Underscore;

  auto tDKtDK =
      cute::partition_fragment_C(typename Kernel::TiledMmaDSQ{},
                                 cute::select<0, 1>(typename Kernel::TileShapeDSQ{}))(
          cute::make_coord(X{}, X{}), cute::_0{}, cute::_0{});
  tDKtDK.data() = Kernel::TmemAllocation::kDK;

  auto tDVtDV =
      cute::partition_fragment_C(typename Kernel::TiledMmaPDO{},
  ...
  auto cDK =
      cute::make_identity_tensor(cute::take<0, 2>(typename Kernel::TileShapeDSQ{}));
  ...
  auto cDV =
      cute::make_identity_tensor(cute::take<0, 2>(typename Kernel::TileShapePDO{}));

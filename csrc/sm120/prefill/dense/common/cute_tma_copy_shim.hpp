...
CUTE_HOST_DEVICE void tma_copy(Traits const& traits, Src const& src, Dst dst) {
  cute::copy(traits, src, dst);
}

template <class CopyTraits, class... Args, class Src, class Dst>
CUTE_HOST_DEVICE void tma_copy(cute::Copy_Atom<CopyTraits, Args...> const& copy_atom,
                               Src const& src, Dst dst) {
  cute::copy(copy_atom, src, dst);
}

template <class CopyAtom, class TV, class Tiler, class Src, class Dst>
CUTE_HOST_DEVICE void tma_copy(cute::TiledCopy<CopyAtom, TV, Tiler> const& tiled_copy,
                               Src const& src,
                               Dst dst) {
